...
- Environment variables: `FRONTEND_URL` on Server, `NEXT_PUBLIC_SERVER_URL` on Frontend (falling back to `https://framesync-server.onrender.com`).
- Permissive CORS enabled in `server/src/index.ts` for Socket.IO and Fastify routes.
